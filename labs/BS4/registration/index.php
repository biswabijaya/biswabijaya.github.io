...
if (isset($_POST['login']) && $_POST['login']=="Login" ){
	...
	$result = mysqli_query($mysqli, "SELECT * FROM students Where emailid='$email'");
		 while($res = mysqli_fetch_array($result))
				 {
					 if (password_verify($pass, $res['pass']))
	 			 		{ $_SESSION['email'] = $res['emailid'];
							$_SESSION['regno'] = $res['studentid'];
							$_SESSION['class'] = $res['class'];
						  header("Location: admission.php");
						  break;
	 					}
						$error="pass";
						$errormsg="wrongpassword";
	 		 	 }
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-10 col-sm-offset-1">
                                        <div class="form-group">
                                            ...
                                            <input name="pass" type="text" class="form-control" placeholder="Enter Mailed Password" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
